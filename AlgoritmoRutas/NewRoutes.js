...
function calcularTiempoLlegada(ruta, matriz) {
    let tiempos = [0];  // El tiempo de salida del depósito es 0
    for (let i = 1; i < ruta.length; i++) {
        let tiempoAnterior = tiempos[i - 1];
        let tiempoViaje = matriz[ruta[i - 1]][ruta[i]];
        tiempos.push(tiempoAnterior + tiempoViaje);
    }
    return tiempos;
}

function verificarVentanasTiempo(ruta, tiemposLlegada, ventanasTiempo) {
    for (let i = 1; i < ruta.length - 1; i++) {
        let cliente = ruta[i];
        let [inicioVentana, finVentana] = ventanasTiempo[cliente];
        if (tiemposLlegada[i] < inicioVentana || tiemposLlegada[i] > finVentana) {
            return false;
        }
    }
    return true;
...
function generarRutas(distanceMatrix, vehicles, clients, demandas, capacidad, ventanasTiempo){
    let rutas = [];
    let savings = [];
    ...
    for(let i=0; i < savings.length; i++){
        ...
        if(ruta_i == null && ruta_j == null){
            ...
            if(calcularCargaRuta(nuevaRuta, demandas) <= capacidad){
                let tiemposLlegada = calcularTiempoLlegada(nuevaRuta, distanceMatrix);
                if (verificarVentanasTiempo(nuevaRuta, tiemposLlegada, ventanasTiempo)) {
...
[60, 100], [70, 110], [80, 120], [90, 130]
];

generarRutas(matriz, vehicles, clients, demandas, capacidad, ventanasTiempo);
