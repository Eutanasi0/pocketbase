...
//Nuevo, ente teoría
function comprobarVentanasTiempo(ruta, distancias, ventanasTiempo, tiempoServicio) {
    let tiempoActual = 0;
    for (let i = 1; i < ruta.length - 1; i++) {
        tiempoActual += distancias[ruta[i - 1]][ruta[i]];
        if (tiempoActual < ventanasTiempo[ruta[i]][0]) {
            tiempoActual = ventanasTiempo[ruta[i]][0];
        }
        if (tiempoActual > ventanasTiempo[ruta[i]][1]) {
            return false;
        }
        tiempoActual += tiempoServicio[ruta[i]];
    }
    return true;
}

// Cambiado para las ventanas de tiempo
function generarRutas(distanceMatrix, vehicles, clients, demandas, capacidad, ventanasTiempo, tiempoServicio) {
    let rutas = [];
    let savings = [];
    ...
    for (let i = 0; i < savings.length; i++) {
        ...
        if (ruta_i == null && ruta_j == null) {
            ...
            if (carga <= capacidad && comprobarVentanasTiempo(nuevaRuta, distanceMatrix, ventanasTiempo, tiempoServicio)) {
        ...
        } else if (ruta_i != null && ruta_j != null && ruta_i != ruta_j) {
            let condicion1 = comprobarPuntoInterior(ruta_i, savings[i][1]);
...
const tiempoServicio = [0, 10, 10, 10, 10, 10, 10, 10, 10, 10];

generarRutas(matriz, vehicles, clients, demandas, capacidad, ventanasTiempo, tiempoServicio);
