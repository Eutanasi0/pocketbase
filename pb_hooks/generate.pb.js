routerAdd('POST', '/generate', (c) => {
  ...
  for (const [route_index, raw_route] of raw_plan.entries()) {
    ...
    for (let w_index = 1; w_index < raw_route.length - 2; w_index++) {
      ...
      const distance = distance_matrix[raw_route[w_index]][raw_route[w_index + 1]]
      const time = res.json.rows[raw_route[w_index]].elements[raw_route[w_index + 1]].duration.value
